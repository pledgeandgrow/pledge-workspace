...
export function ConventionDeStageEditor() {
  ...
  const [internshipInfo, setInternshipInfo] = useState<InternshipInfo>({
    ...
    tasks:
      "Développement d'applications web, intégration de maquettes, tests et débogage",
  ...
  const [educationalInfo, setEducationalInfo] = useState<EducationalInfo>({
    ...
    year: "",
    objectives:
      "Acquérir une expérience professionnelle dans le domaine du développement web",
  ...
  const exportToPdf = async () => {
    if (documentRef.current) {
      ...
      try {
        ...
        // Get all sections
        const sections = documentRef.current.querySelectorAll(
          ".convention-section"
        );
