...
const CahierDesChargesPreviewComponent = forwardRef<HTMLDivElement, CahierDesChargesPreviewProps>(
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex justify-between text-sm">
            ...
            <p>Statut: {
              data.projectInfo.status === "draft" ? "Brouillon" :
              data.projectInfo.status === "review" ? "En revue" :
              data.projectInfo.status === "approved" ? "Approuvé" :
              "Terminé"
            }</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <p><strong>Contact:</strong> {data.clientInfo.contactPerson}</p>
            <p><strong>Email:</strong> {data.clientInfo.email}</p>
            <p><strong>Téléphone:</strong> {data.clientInfo.phone}</p>
            <p><strong>Adresse:</strong> {data.clientInfo.address}</p>
            {data.clientInfo.website && <p><strong>Site Web:</strong> {data.clientInfo.website}</p>}
          </div>
          <div>
            <h2 className="text-xl font-bold border-b-2 border-gray-300 pb-2 mb-4">Prestataire</h2>
            <p><strong>Nom:</strong> {data.companyInfo.name}</p>
            <p><strong>Contact:</strong> {data.companyInfo.contactName}</p>
            <p><strong>Adresse:</strong> {data.companyInfo.address}</p>
            <p><strong>Code postal et ville:</strong> {data.companyInfo.zipCity}</p>
            <p><strong>Pays:</strong> {data.companyInfo.country}</p>
            <p><strong>N° d&apos;entreprise:</strong> {data.companyInfo.companyNumber}</p>
            <p><strong>N° TVA:</strong> {data.companyInfo.vatNumber}</p>
            <p><strong>Email:</strong> {data.companyInfo.email}</p>
            <p><strong>Téléphone:</strong> {data.companyInfo.phone}</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {data.teamMembers.map((member) => (
                <div key={member.id} className="border p-4 rounded-md">
                  ...
                  <p><strong>Responsabilités:</strong> {member.responsibilities}</p>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-white ${
                      ...
                      {objective.priority === "high" ? "Priorité haute" :
                       objective.priority === "medium" ? "Priorité moyenne" :
                       "Priorité basse"}
                    </span>
                  </div>
                  <p className="mt-2">{objective.description}</p>
                  {objective.measurableOutcome && (
                    <p className="mt-2"><strong>Résultat mesurable:</strong> {objective.measurableOutcome}</p>
                  )}
                </div>
              ))}
            </div>
              ...
              {data.deliverables.map((deliverable) => (
                <div key={deliverable.id} className="border p-4 rounded-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    <p><strong>Format:</strong> {deliverable.format}</p>
                    <p><strong>Date d&apos;échéance:</strong> {formatDate(deliverable.dueDate)}</p>
                  </div>
                  <p className="mt-2"><strong>Critères d&apos;acceptation:</strong> {deliverable.acceptanceCriteria}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              {data.milestones.map((milestone) => (
                <div key={milestone.id} className="border p-4 rounded-md">
                  ...
                  {milestone.deliverables.length > 0 && (
                    <div className="mt-2">
                      <p><strong>Livrables associés:</strong></p>
              ...
              {data.functionalRequirements.map((requirement) => (
                <div key={requirement.id} className="border p-4 rounded-md">
                  <div className="flex justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-white ${
                      requirement.priority === "critical" ? "bg-red-600" :
                      requirement.priority === "high" ? "bg-red-500" :
                      requirement.priority === "medium" ? "bg-yellow-500" :
                      "bg-green-500"
                    }`}>
                      {requirement.priority === "critical" ? "Critique" :
                       requirement.priority === "high" ? "Priorité haute" :
                       requirement.priority === "medium" ? "Priorité moyenne" :
                       "Priorité basse"}
                    </span>
                  </div>
                  {requirement.category && <p className="text-sm text-gray-600 mt-1">Catégorie: {requirement.category}</p>}
                  <p className="mt-2">{requirement.description}</p>
                  <p className="mt-2"><strong>Critères d&apos;acceptation:</strong> {requirement.acceptanceCriteria}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {data.technicalRequirements.map((requirement) => (
                <div key={requirement.id} className="border p-4 rounded-md">
                  ...
                  {requirement.category && <p className="text-sm text-gray-600 mt-1">Catégorie: {requirement.category}</p>}
                  <p className="mt-2">{requirement.description}</p>
                  <p className="mt-2"><strong>Implémentation:</strong> {requirement.implementation}</p>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={requirement.id} className="border p-4 rounded-md">
                  <div className="flex justify-between">
                    ...
                    <span className="px-2 py-1 rounded text-white bg-blue-500">
                      {requirement.category === "performance" ? "Performance" :
                       requirement.category === "security" ? "Sécurité" :
                       requirement.category === "usability" ? "Utilisabilité" :
                       requirement.category === "reliability" ? "Fiabilité" :
                       requirement.category === "maintainability" ? "Maintenabilité" :
                       "Autre"}
                    </span>
                  </div>
                  <p className="mt-2">{requirement.description}</p>
                  <p className="mt-2"><strong>Critères de mesure:</strong> {requirement.measurementCriteria}</p>
                </div>
              ))}
            </div>
          </div>
                ...
                <tbody>
                  ...
                  <tr className="font-bold bg-gray-100">
                    ...
                    <td className="border p-2 text-right">
                      {data.budget.breakdown.reduce((sum: number, item: BudgetItem) => sum + item.amount, 0).toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          {data.budget.paymentSchedule.length > 0 && (
            <div>
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {data.budget.paymentSchedule.map((milestone: PaymentMilestone) => (
                    <tr key={milestone.id}>
                      <td className="border p-2">{milestone.description}</td>
                      <td className="border p-2">{formatDate(milestone.dueDate)}</td>
                      <td className="border p-2">{milestone.conditions}</td>
                      <td className="border p-2 text-right">{milestone.amount.toFixed(2)}</td>
                    </tr>
                  ))}
                  <tr className="font-bold bg-gray-100">
                    <td className="border p-2" colSpan={3}>Total</td>
                    <td className="border p-2 text-right">
                      {data.budget.paymentSchedule.reduce((sum: number, milestone: PaymentMilestone) => sum + milestone.amount, 0).toFixed(2)}
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <p><strong>Date de début:</strong> {formatDate(data.timeline.startDate)}</p>
            <p><strong>Date de fin:</strong> {formatDate(data.timeline.endDate)}</p>
          </div>
                ...
                <div key={risk.id} className="border p-4 rounded-md">
                  <div className="flex justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-white ${
                      ...
                      Impact: {risk.impact === "high" ? "Élevé" :
                              risk.impact === "medium" ? "Moyen" :
                              "Faible"}
                    </span>
                  </div>
                  <p className="mt-2">{risk.description}</p>
                  <p className="mt-2"><strong>Probabilité:</strong> {
                    risk.probability === "high" ? "Élevée" :
                    risk.probability === "medium" ? "Moyenne" :
                    "Faible"
                  }</p>
                  <p className="mt-2"><strong>Stratégie de mitigation:</strong> {risk.mitigation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="border p-4 rounded-md">
              ...
              <p><strong>Nom:</strong> {data.approval.clientName}</p>
              <p><strong>Titre:</strong> {data.approval.clientTitle}</p>
              <p><strong>Date:</strong> {formatDate(data.approval.clientSignatureDate)}</p>
            ...
            <div className="border p-4 rounded-md">
              ...
              <p><strong>Nom:</strong> {data.approval.companyName}</p>
              <p><strong>Titre:</strong> {data.approval.companyTitle}</p>
              <p><strong>Date:</strong> {formatDate(data.approval.companySignatureDate)}</p>
              <div className="mt-4 pt-4 border-t border-gray-300">
                <p>Signature:</p>
                <div className="h-16 border border-dashed border-gray-300 mt-2"></div>
              </div>
